...
//            size = GameTools::HB_POINT_PIXELS_TO_POINTS(size);
    ...
    CCLabelTTF* createLabelTTFWithScript(CCDictionary* data)
    {
        ...
        {
            //代表预定义字体大小
            int size = sizeof(g_custom_font_size)/sizeof(g_custom_font_size[0]);
            int index = -intFontSize;
            if(index < size)
            {
                intFontSize = g_custom_font_size[index];
            }
        }
//        else {
//            #if (CC_TARGET_PLATFORM != CC_PLATFORM_ANDROID)
...
//        }
         ...
         //字体颜色
        ccColor3B color = g_color_white;
        //颜色
        string sColor = string(valueForKey("color", data));
        int iColor = atoi(sColor.c_str());
        int size = sizeof(g_custom_color)/sizeof(g_custom_color[0]);
        if (iColor > 0 && iColor <= size) {
            color = g_custom_color[iColor - 1];
        } else {
            if (sColor != "") {

...
void CGamePlayer::appendCardBag()
{
    m_gGamePlayerData->appendCardBagAppend();
}
...
void CGamePlayer::addFriendly(int inFriendly)
{
    m_gGamePlayerData->m_nFriendly += inFriendly;
    
}
void CGamePlayer::subFriendly(int inFriendly)
{
    m_gGamePlayerData->m_nFriendly -= inFriendly;
...
// reward走邮件：
void CGamePlayer::addEmails(CCDictionary *inboxinfoDict)
{
    // inbox_info:
    string teamStrType;
    if (inboxinfoDict)
    {
        teamStrType=typeid(*inboxinfoDict).name();
        if(teamStrType.find("CCDictionary")!=std::string::npos)
        {

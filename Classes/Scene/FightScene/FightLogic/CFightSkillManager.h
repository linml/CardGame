...
static void FUNCTIONNAME(CFightCard *pCard,vector<CFightCard *>FightCard,vector<CFightCard *>MonsterCard,int FightIndex,int MonsterIndex,CSkillData *pSkill,EN_ATKFIGHT_INDEX enAtkIndex);
...
public:
    ...
    static void logicSkill_Putong(CFightCard *pCard,vector< CFightCard *>pFightCard,vector< CFightCard *>pMonterCard,int FightIndex,int MonsterIndex,CSkillData *pSkill,EN_ATKFIGHT_INDEX enAtkIndex);
    static string logicSkillFight(CFightCard *pCard,vector< CFightCard *>pFightCard,vector< CFightCard *>pMonterCard,int FightIndex,int MonsterIndex,CSkillData *pSkill,int paramid,EN_ATKFIGHT_INDEX enatkIndex,bool isDirenTuandui=true,bool isQunti=false);//如果是团队的话 判断是给哪个团队加血
    
    STATICSKILLFUNCTION(logicSkill_0);
...
public:
   ...
   //普通伤害计算
    ...
    static void appendVector(EN_ANIMATIONTYPEREFACTOR entype,EN_ATKFIGHT_INDEX enAtkIndex,
                             int skillId,
                             int nATKindex,
                             int nDefIndex,
